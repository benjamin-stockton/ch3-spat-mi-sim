...
x1 <- simulation(N_sim, N_sample, init_seed, M, methods, pop_pars, miss_pars, mc.cores = N_sim)

out_path <- file.path("sim-results")


saveRDS(x1, file = paste0(out_path, "/test-sim-results-mar-spat-mi-", Sys.Date(), "-sim_setting-", 3, ".rds"))

x2 <- x1 |>
    dplyr::bind_rows()
f_out <- paste0(out_path, "/test-sim-results-mar-spat-mi-sim_setting-", 3, ".csv")
